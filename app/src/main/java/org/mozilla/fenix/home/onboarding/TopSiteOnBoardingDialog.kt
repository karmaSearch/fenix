...
import org.mozilla.fenix.databinding.OnboardingDialogTopsiteBinding
import org.mozilla.fenix.ext.settings
import org.mozilla.fenix.home.topsites.TopSitePagerViewHolder

class TopSiteOnBoardingDialog(val recyclerView: RecyclerView) {
    ...
    private fun createJumpCRF(anchor: View): Dialog? {
        ...
        if (!context.settings().shouldShowTopSiteCompanion) {
            return null
        }
        val anchorPosition = IntArray(2)
        val popupBinding = OnboardingDialogTopsiteBinding.inflate(LayoutInflater.from(context))
